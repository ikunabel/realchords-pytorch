...
Each --model path is also resolved to that run's actual lowest-val-loss checkpoint by
default (via Lightning's own ModelCheckpoint callback state, not the last-saved
checkpoint -- training keeps running well past the actual minimum). Pass
--no_best_checkpoint to use exactly the given file instead.
...
Usage:
    python scripts/eval/recall/eval_reward_recall.py --reward_type contrastive \\
    ...
    python scripts/eval/recall/eval_reward_recall.py --reward_type discriminative \\
...
def resolve_best_checkpoint(path: str) -> str:
    """Resolve a checkpoint path to that run's actual lowest-val-loss checkpoint.

    `path` may be a specific `step=*.ckpt` file or a run directory -- either way,
    the run directory is scanned for all `step=*.ckpt` files, the one with the
    highest step is loaded (its Lightning `ModelCheckpoint` callback state has
    the most complete, cumulative "best so far" tracking for the whole run:
    `checkpoint["callbacks"][<ModelCheckpoint key>]["best_model_path"]`), and
    that callback's `best_model_path` is used instead of blindly assuming the
    last-saved checkpoint is the best one (it usually isn't -- training keeps
    running after the actual minimum). Falls back to the last checkpoint if no
    such callback state exists (e.g. single-checkpoint HF reference dirs).
    """
    p = Path(path)
    run_dir = p.parent if p.is_file() else p
    ckpts = sorted(
        run_dir.glob("step=*.ckpt"), key=lambda f: int(f.stem.split("=")[1])
    )
    if not ckpts:
        raise FileNotFoundError(f"No step=*.ckpt files found in {run_dir}")
    last_ckpt = ckpts[-1]

    raw = torch.load(last_ckpt, map_location="cpu", weights_only=False)
    for key, state in raw.get("callbacks", {}).items():
        if "ModelCheckpoint" in key and state.get("best_model_path"):
            best_path = run_dir / Path(state["best_model_path"]).name
            if best_path.exists():
...
def main():
    ...
    parser.add_argument(
        "--no_best_checkpoint",
        action="store_true",
        help="Use exactly the checkpoint file given in --model, instead of resolving "
        "to that run's actual lowest-val-loss checkpoint (the default) via "
        "Lightning's own ModelCheckpoint callback state.",
    )
    ...
    for label, model_path in models_to_run:
        if not args.no_best_checkpoint:
            resolved_path = resolve_best_checkpoint(model_path)
            if resolved_path != model_path:
                print(f"  {label}: resolved to best checkpoint {resolved_path}")
