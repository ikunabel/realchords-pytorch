...
``python scripts/eval/custom_eval/run_custom_eval.py gt_all midi_gt_all`` first) -- this
script doesn't generate anything, only reads
``logs/custom_eval/gt/<dataset>_<split>/full_songs/midi/*.mid``.

Usage::

    python scripts/eval/custom_eval/clamp2_dataset_probe.py
    python scripts/eval/custom_eval/clamp2_dataset_probe.py --max_per_dataset 20
    python scripts/eval/custom_eval/clamp2_dataset_probe.py --genres "a pop song" "a jazz song"
"""
...
def discover_dataset_midi_dirs(gt_root: Path, split_mode: str) -> Dict[str, Path]:
    ...
    the full export the ``gt_*`` presets in configs/custom_eval/
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out_dir",
        type=str,
        default="logs/custom_eval/clamp2_probe",
