#!/usr/bin/env python3
"""Aggregate per-dataset `means.json` files from `gt_all` into
one cross-dataset comparison table.

The `gt_*` presets in configs/custom_eval/ (run via
`scripts/eval/custom_eval/run_custom_eval.py`) each write to
`logs/custom_eval/gt/<dataset>_<split>/{cropped_songs,full_songs}/means.json`
...
Usage::
    ...
    python scripts/eval/custom_eval/summarize_custom_metrics.py
    python scripts/eval/custom_eval/summarize_custom_metrics.py --gt_root logs/custom_eval/gt \\
        --out_dir logs/custom_eval/gt/gt_summary

By default, CSVs are written to <gt_root>/gt_summary (nested under gt_root,
not a sibling of it) -- pass --out_dir to override, or --no_csv to only print.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Where to write cropped_songs.csv / full_songs.csv / model_comparison.csv. "
             "Defaults to <gt_root>/gt_summary (nested under gt_root).",
    )
    parser.add_argument(
        "--no_csv",
        action="store_true",
        help="Only print the tables; don't write CSVs at all.",
    )
...
def main() -> None:
    ...
    if not gt_root.exists():
        raise SystemExit(f"{gt_root} does not exist -- run gt_all first")
    ...
    if not args.no_csv:
        # Nested under gt_root by default (not a sibling of it), so the
        # summary lives alongside the per-dataset dirs it was built from.
        out_dir = Path(args.out_dir) if args.out_dir else gt_root / "gt_summary"
