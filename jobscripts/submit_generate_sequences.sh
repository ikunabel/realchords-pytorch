...
#
# Submit:
#   sbatch jobscripts/submit_generate_sequences.sh
#
...
#   MAX_PARALLEL=2 sbatch jobscripts/submit_generate_sequences.sh
#
...
LOG_DIR="jobscripts/slurm_logs/${SLURM_JOB_NAME}"

source jobscripts/generate_sequences_functions.sh
...
if (( ${#gen_fns[@]} == 0 )); then
  echo "ERROR: gen_fns is empty. Add function names to jobscripts/submit_generate_sequences.sh"
  exit 2
fi
