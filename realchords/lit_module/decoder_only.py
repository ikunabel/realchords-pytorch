"""Lightning module for training the decoder only generative model."""
...
create_weighted_joint_dataset = bind(create_weighted_joint_dataset, without_prefix=True)
...
class LitDecoder(BaseLightningModel):
    def __init__(
        ...
        )  # -2 for the bos token and eos token
    ...
    def get_inputs(self, batch):
        ...
        # Change to not predict EOS token
        # dec_inputs_mask = batch["targets_mask"][:, 1:]
        dec_inputs_mask = (targets != self.pad_token) & (targets != self.eos_token)
    ...
    def sample_and_log(self, targets, song_urls):
        ...
        conditions_mask = conditions != self.pad_token
        mask = torch.zeros(
            (conditions.shape[0], self.max_gen_seq_len),
            dtype=torch.bool,
            device=conditions.device,
        )
        mask[:, 0::2][:, : conditions_mask.shape[1]] = conditions_mask
        mask[:, 1::2][:, : conditions_mask.shape[1]] = conditions_mask
        # mask: True for unmasked tokens
        ...
        decoder_preds.masked_fill_(~mask, self.pad_token)

        decoder_preds_uncond = self.model.generate(
            gen_inputs,
            seq_len=self.max_gen_seq_len,
            cache_kv=True,
            eos_token=self.eos_token,
        ...
        print(f"Time to generate: {time.time() - curr_time}")

        # We don't log audio and image into table because there will be bugs
        table_text = wandb.Table(
            columns=["Index", "Array GT", "Array Gen", "Array Gen Uncond"]
        )
        for i in range(min(self.max_log_examples, len(targets))):
            decoder_pred = decoder_preds[i].cpu().numpy()
            decoder_pred_uncond = decoder_preds_uncond[i].cpu().numpy()
            target = targets[i].cpu().numpy()
            # Convert array to text
            array_pred_text = str(decoder_pred)
            array_pred_uncond_text = str(decoder_pred_uncond)
            array_gt_text = str(target)
            table_text.add_data(
                i, array_gt_text, array_pred_text, array_pred_uncond_text
            )
            ...
            try:
                midi_gen_uncond = self.output_to_midi(decoder_pred_uncond)
                self.log_midi(midi_gen_uncond, suffix=f"gen_uncond_{i}")
            except Exception as e:
                logger.error(f"Error converting to MIDI: {e}")
